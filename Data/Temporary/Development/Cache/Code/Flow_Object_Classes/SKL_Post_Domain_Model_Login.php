...
class Login extends Login_Original implements \TYPO3\Flow\Object\Proxy\ProxyInterface, \TYPO3\Flow\Persistence\Aspect\PersistenceMagicInterface {
		...
		$this->Flow_Aop_Proxy_buildMethodsAndAdvicesArray();
			...
			if (isset($this->Flow_Aop_Proxy_methodIsInAdviceMode['__construct'])) {
				...
				if (isset($this->Flow_Aop_Proxy_targetMethodsAndGroupedAdvices['__construct']['TYPO3\Flow\Aop\Advice\BeforeAdvice'])) {
					...
					$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'SKL\Post\Domain\Model\Login', '__construct', $methodArguments);
		...
		$result = NULL;
			...
			try {
				...
				if (isset($this->Flow_Aop_Proxy_targetMethodsAndGroupedAdvices['__clone']['TYPO3\Flow\Aop\Advice\BeforeAdvice'])) {
					...
					$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'SKL\Post\Domain\Model\Login', '__clone', $methodArguments);
					foreach ($advices as $advice) {
						$advice->invoke($joinPoint);
					}
				...
				if (isset($this->Flow_Aop_Proxy_targetMethodsAndGroupedAdvices['__clone']['TYPO3\Flow\Aop\Advice\AfterReturningAdvice'])) {
					...
					$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'SKL\Post\Domain\Model\Login', '__clone', $methodArguments, NULL, $result);
					foreach ($advices as $advice) {
						$advice->invoke($joinPoint);
					}
	...
	$reflectedClass = new \ReflectionClass('SKL\Post\Domain\Model\Login');
	$allReflectedProperties = $reflectedClass->getProperties();
	foreach ($allReflectedProperties as $reflectionProperty) {
		$propertyName = $reflectionProperty->name;
		if (in_array($propertyName, array('Flow_Aop_Proxy_targetMethodsAndGroupedAdvices', 'Flow_Aop_Proxy_groupedAdviceChains', 'Flow_Aop_Proxy_methodIsInAdviceMode'))) continue;
		if (isset($this->Flow_Injected_Properties) && is_array($this->Flow_Injected_Properties) && in_array($propertyName, $this->Flow_Injected_Properties)) continue;
		if ($reflectionProperty->isStatic() || $reflectionService->isPropertyAnnotatedWith('SKL\Post\Domain\Model\Login', $propertyName, 'TYPO3\Flow\Annotations\Transient')) continue;
		...
		if (is_object($this->$propertyName) && !$this->$propertyName instanceof \Doctrine\Common\Collections\Collection) {
			if ($this->$propertyName instanceof \Doctrine\ORM\Proxy\Proxy) {
				...
				$varTagValues = $reflectionService->getPropertyTagValues('SKL\Post\Domain\Model\Login', $propertyName, 'var');
...
# PathAndFilename: /var/www/html/flow-login/Packages/Application/SKL.Post/Classes/SKL/Post/Domain/Model/Login.php
#