...
class FormController extends FormController_Original implements \TYPO3\Flow\Object\Proxy\ProxyInterface {
	...
	public function __construct() {
		if ('SKL\Post\Controller\FormController' === get_class($this)) {
	...
	$reflectedClass = new \ReflectionClass('SKL\Post\Controller\FormController');
	$allReflectedProperties = $reflectedClass->getProperties();
	foreach ($allReflectedProperties as $reflectionProperty) {
		$propertyName = $reflectionProperty->name;
		if (in_array($propertyName, array('Flow_Aop_Proxy_targetMethodsAndGroupedAdvices', 'Flow_Aop_Proxy_groupedAdviceChains', 'Flow_Aop_Proxy_methodIsInAdviceMode'))) continue;
		if (isset($this->Flow_Injected_Properties) && is_array($this->Flow_Injected_Properties) && in_array($propertyName, $this->Flow_Injected_Properties)) continue;
		if ($reflectionProperty->isStatic() || $reflectionService->isPropertyAnnotatedWith('SKL\Post\Controller\FormController', $propertyName, 'TYPO3\Flow\Annotations\Transient')) continue;
		...
		if (is_object($this->$propertyName) && !$this->$propertyName instanceof \Doctrine\Common\Collections\Collection) {
			if ($this->$propertyName instanceof \Doctrine\ORM\Proxy\Proxy) {
				...
				$varTagValues = $reflectionService->getPropertyTagValues('SKL\Post\Controller\FormController', $propertyName, 'var');
	 ...
	 */
	 private function Flow_Proxy_injectProperties() {
		...
		$this->formRepository = \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->getInstance('SKL\Post\Domain\Repository\FormRepository');
		if ($this->formRepository === NULL) {
			$this->formRepository = \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->getLazyDependencyByHash('8bfefe2f482ab9d5369332957f7add37', $formRepository_reference);
			if ($this->formRepository === NULL) {
				$this->formRepository = \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->createLazyDependency('8bfefe2f482ab9d5369332957f7add37',  $formRepository_reference, 'SKL\Post\Domain\Repository\FormRepository', function() { return \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->get('SKL\Post\Domain\Repository\FormRepository'); });
			}
		}
...
# PathAndFilename: /var/www/html/flow-login/Packages/Application/SKL.Post/Classes/SKL/Post/Controller/FormController.php
#